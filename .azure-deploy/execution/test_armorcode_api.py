...
try:
    response = requests.post(f"{base_url}/graphql", headers=headers, json=graphql_query, timeout=30)
...
print("-" * 80)
post_body = {"severity": ["HIGH", "CRITICAL"], "status": ["Open", "In Progress"], "limit": 5}

try:
    response = requests.post(f"{base_url}/api/findings", headers=headers, json=post_body, timeout=30)
    print(f"Status: {response.status_code}")
    print(f"Response: {response.text[:500]}")
    if response.status_code == 200:
...
try:
    response = requests.post(f"{base_url}/api/v1/findings", headers=headers, json=post_body, timeout=30)
    print(f"Status: {response.status_code}")
    print(f"Response: {response.text[:500]}")
    if response.status_code == 200:
...
try:
    response = requests.post(f"{base_url}/api/findings", headers=alt_headers, json=post_body, timeout=30)
    print(f"Status: {response.status_code}")
    print(f"Response: {response.text[:500]}")
    if response.status_code == 200:
...
try:
    response = requests.post(f"{base_url}/graphql", headers=headers, json=graphql_products, timeout=30)
    print(f"Status: {response.status_code}")
    print(f"Response: {response.text[:500]}")
    if response.status_code == 200:
