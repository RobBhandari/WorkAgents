...
async def test_get_work_items_rejects_large_batches():
    ...
    with pytest.raises(ValueError, match="API limit is 200 per call"):
        await get_work_items_by_ids(organization="contoso", ids=large_batch)
...
async def test_get_work_items_handles_empty_list():
    ...
    # Execute
    result = await get_work_items_by_ids(organization="contoso", ids=[])
...
async def test_get_work_items_with_field_filtering():
    ...
    mock_result = {
        "count": 2,
        "value": [{"id": 1001, "fields": {"System.Title": "Bug 1"}}, {"id": 1002, "fields": {"System.Title": "Bug 2"}}],
    }

    with patch("skills.ado_skill.tools.get_work_items_by_ids.AzureDevOpsRESTClient") as mock_client_cls:
        ...
        result = await get_work_items_by_ids(
            organization="contoso", ids=[1001, 1002], fields=["System.Id", "System.Title"]
        )

        # Verify
        assert result == mock_result
        mock_client.get_work_items.assert_called_once_with(ids=[1001, 1002], fields=["System.Id", "System.Title"])
...
async def test_get_work_items_success():
    ...
    mock_result = {
        ...
        "value": [
            ...
            {"id": 1002, "fields": {"System.Title": "Feature request", "System.State": "New"}},
            {"id": 1003, "fields": {"System.Title": "Fix typo", "System.State": "Closed"}},
        ],
    }
    ...
    with patch("skills.ado_skill.tools.get_work_items_by_ids.AzureDevOpsRESTClient") as mock_client_cls:
        ...
        # Execute
        result = await get_work_items_by_ids(organization="contoso", ids=[1001, 1002, 1003])
