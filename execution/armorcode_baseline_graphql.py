"""
ArmorCode Baseline Creator - GraphQL Version

Uses GraphQL API with proper filtering to create accurate baseline.
...
def fetch_findings_for_product_graphql(api_key, base_url, product_id, product_name):
    """Fetch HIGH+CRITICAL findings with OPEN+CONFIRMED status via GraphQL."""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'data' in data and 'findings' in data['data']:
                    ...
                    if page == 1:
                        total = page_info.get('totalElements', len(findings))
...
def create_baseline(force=False):
    """Create baseline snapshot using GraphQL API."""
    ...
    logger.info("="*70)
    logger.info("ARMORCODE BASELINE CREATION - GraphQL API")
    logger.info("="*70)
    logger.info(f"Snapshot Date: {datetime.now().strftime('%Y-%m-%d')}")
    logger.info(f"Baseline Reference Date: {baseline_date}")
    logger.info(f"Status Filter: OPEN + CONFIRMED")
    logger.info(f"Severity Filter: HIGH + CRITICAL")
    ...
    # Step 2: Fetch findings for each product via GraphQL
    all_findings = []
    product_counts = {}
    ...
    for product in product_data:
        ...
        findings = fetch_findings_for_product_graphql(api_key, base_url, product_id, product_name)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Create ArmorCode baseline snapshot using GraphQL')
