...
def load_baseline(project_key: str = None) -> dict:
    ...
    if not os.path.exists(baseline_file):
        raise FileNotFoundError(
            ...
            f"Please run: python execution/ado_baseline.py" + (f" --project-key {project_key}" if project_key else "")
        )
...
def calculate_doe_metrics(
    organization_url: str, project_name: str, pat: str, week_number: int = None, project_key: str = None
) -> dict:
    """
    ...
    report = {
        ...
        "expected": {"open": expected_open, "delta": delta},
        "status": {"indicator": status, "emoji": STATUS_EMOJIS[status]},
        "timeline": {
            "weeks_elapsed": week_number,
...
def format_report_text(report: dict) -> str:
    ...
    output.append("")
    output.append(
        f"New: {report['current']['new']} | Closed: {report['current']['closed']} | Net burn: {report['current']['net_burn']:+d}"
    )
...
def parse_arguments():
    ...
    """
    parser = argparse.ArgumentParser(description="Calculate DOE metrics and track progress toward bug reduction target")

    parser.add_argument(
        "--week-number", type=int, help="Override week number (default: auto-calculate from baseline date)"
    )
    ...
    parser.add_argument("--output-file", help="Save report to file (in addition to console output)")
