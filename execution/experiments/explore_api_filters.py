...
response = post(f"{base_url}/api/findings", headers=headers, json={"limit": 10000}, timeout=60)
data = response.json()
...
response = post(f"{base_url}/api/findings", headers=headers, json={"severity": ["HIGH", "CRITICAL"], "limit": 10000}, timeout=60)
data = response.json()
...
response = post(f"{base_url}/api/findings", headers=headers, json={"severity": ["HIGH", "CRITICAL"], "status": ["Open", "In Progress"], "limit": 10000}, timeout=60)
data = response.json()
