...
org_url = get_config().get("ADO_ORGANIZATION_URL")
...
all_ids = {item.id for item in all_result.work_items} if all_result.work_items else set()
exclude_ids = {item.id for item in exclude_result.work_items} if exclude_result.work_items else set()
include_ids = {item.id for item in include_result.work_items} if include_result.work_items else set()
...
if missing_ids:
    ...
    bugs = wit_client.get_work_items(
        list(missing_ids), fields=["System.Id", "System.Title", "System.AreaPath", "System.State", "System.CreatedDate"]
    )
    ...
    print("\n✓ No bugs missing - filters are complete!")
...
if all_result.work_items:
    ...
    all_bugs = wit_client.get_work_items(all_bug_ids, fields=["System.Id", "System.AreaPath"])
